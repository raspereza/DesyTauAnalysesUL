...
// Initialization
SynchGenTree::SynchGenTree(TTree *tree) : fChain(0) {
  lock = false;
  ientry = -2;
...
void SynchGenTree::Init(TTree *tree){
  if(!tree) return;
...
// Destructors
SynchGenTree::~SynchGenTree(){
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

// Read methods
void SynchGenTree::ReadInit(TTree *tree)
{
...
void SynchGenTree::ReadReset(){
  ientry = -2;
}

Long64_t SynchGenTree::GetEntries(){
  if (!fChain) return -1;
  
  return fChain->GetEntriesFast();
}

Long64_t SynchGenTree::LoadTree(Long64_t entry){
...
Long64_t SynchGenTree::GetEntry(Long64_t entry){
...
Long64_t SynchGenTree::LoadedEntryId(){
  return ientry;
}
    

void SynchGenTree::Show(Long64_t entry){
...
Int_t SynchGenTree::Cut(Long64_t entry){
...
//Write Methods
void SynchGenTree::WriteInit(TTree *tree) {
  if (!tree) return;
